...
var Tile = (props)=>{
  ...
  // Positioning infromation goes after dash
  const img = config.image.split('-')[0];
...
let World = React.createClass({
  ...
  render: function (){
    ...
    var tiles = _.map(config.objects, (object)=>{
        return <Tile config = {config} tileConfig={object}></Tile>
  });
    var character = <Tile config = {config} tileConfig={config.character}></Tile>
    var buttons = _.map(config.buttons, (button, key)=>{
        return <button  key = {key} onClick = {this.handleAction.bind(this, button)}>{button.name}</button>
  });
